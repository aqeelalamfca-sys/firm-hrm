...
- Schema: `lib/db/src/schema/working_papers.ts` (26 tables incl. all COA, Audit Engine, WP trigger, sampling, analytics, control, evidence, recon tables)
- Variable definitions: `artifacts/api-server/src/data/variable-definitions.ts` (415 variables incl. 96 CY/PY generated, 20 groups). 6 new codes added: `engagement_size`, `tb_line_count`, `tb_total_period_debit`, `tb_total_period_credit`, `tb_opening_balance_aggregate`, `tb_closing_balance_aggregate`. Auto-fill covers 100% of variables with tiered confidence. Template parse (sourceType="template", confidence=1.00) now maps 15+ additional codes: `engagement_size`, TB aggregates, `significant_risk_areas`, `account_level_risk_mapping_done`, `population_value`, `sampling_required`, `sampling_basis`, `tb_balanced_flag`, `account_type`, `control_accounts_identified`, `fraud_risk_flag`, `revenue_fraud_risk_flag`, `management_override_risk_flag`, `books_maintained_properly`, `inventory_records_available`, `bank_statements_available`, `principal_activity`.
...
- **8-stage flow**: Upload → AI Extraction → Data Sheet (COA Engine) → Arranged Data Review → Variables (Lock) → Audit Engine → Head-wise Generation → Export
